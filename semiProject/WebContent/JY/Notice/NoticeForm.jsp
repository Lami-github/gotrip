 ...
 <tiles:insertDefinition name="adminheader" />
	...
	<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
		...
		<table  width="700" align="center">
			<tr>
				<td align="right">
					<a href="/semiProject/JY/NoticeWriteForm.admin">글쓰기</a>
				</td>
			</tr>
		</table>
			...
			<table border="1" width="700" cellpadding="0" cellspacing="0" align="center">
				...
				<c:forEach var="article" items="${articleList}"> <!-- items 콜렉션..... -->
					<tr height="30">
						...
						<td width="250">
							<a href="/semiProject/JY/NoticeDetailForm.admin?num=${article.n_num}&pageNum=${currentPage}">
								${article.n_subject}
							</a>
							
						</td>
						<td align="center" width="150">${article.n_reg_date}</td>
						<td align="center" width="50">${article.n_readcount}</td>
					</tr>
				</c:forEach>
			</table>
		</c:if>
		<div style="text-align:center">
			<c:if test="${count > 0}">
				...
				<c:if test="${startPage > 10}">
					<a
						href="/semiProject/JY/NoticeForm.admin?pageNum=${startPage - 10 }">[이전]</a>
				</c:if>

				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a href="/semiProject/JY/NoticeForm.admin?pageNum=${i}">[${i}]</a>
				</c:forEach>

				<c:if test="${endPage < pageCount}">
					<a href="/semiProject/JY/NoticeForm.admin?pageNum=${startPage + 10}">[다음]</a>
				</c:if>
			</c:if>
