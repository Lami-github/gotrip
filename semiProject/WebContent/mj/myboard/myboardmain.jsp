   ...
   <section id="One" class="wrapper style3">
      <div class="inner">
         <header class="align-center">
            ...
            <a href="semiProject/mj/myboardMain.do"><h2>내가 쓴 글</h2></a>
         </header>
      </div>
   </section>
         ...
         <div class="box">
            <div class="content">
               <table border="0" align="center">
                     ...
                     <td class="asd">
                        <div class="row uniform">
                           ...
                           <div class="3u$ 12u$(small)">
                              <input type=button
                                 onclick="location.href='/semiProject/mj/myboardcommentMain.do'"
                                 value="내가 쓴 댓글" />
                           </div>
                        </div>
                  ...
                  <tr align=center>
                     <td>
                        <table border=1 width=760 cellpadding=0 cellspacing=0 style="text-align:center;">
                           <tr height="30">
                           ...
                           <c:choose>
                              ...
                              <c:otherwise>
                                 ...
                                 <c:forEach var="article" items="${article}">
                                    <tr>
                                       <td><c:out value="${article.num}" /></td>
                                       <td>${option}</td>
                                       <td><c:if test="${option eq 'buyer'}">
                                             <a
                                                href="/semiProject/NA/buyer/buyerBoardContent.do?pageNum=${currentPage}&b_num=${article.num}"><c:out
                                                   value="${article.subject}" /></a>
                                          </c:if> <c:if test="${option eq 'traveler'}">
                                             <a
                                                href="/semiProject/mj/travelerDetail.do?pageNum=${currentPage}&tr_num=${article.num}"><c:out
                                                   value="${article.subject}" /></a>
                                          </c:if> <c:if test="${option eq 'tripreview'}">
                                             <a
                                                href="/semiProject/NA/tripreView/tripreViewContent.do?pageNum=${currentPage}&t_num=${article.num}"><c:out
                                                   value="${article.subject}" /></a>
                                          </c:if></td>
                                       <td><c:out value="${article.reg_date}" /></td>
                     ...
                     <tr align=center>
                        <td class="asd"><c:set var="pageCount"
                           ...
                           <c:set var="startPage" value="${result * 10 +1 }" /> <c:set
                              ...
                              <a
                                 href="/semiProject/mj/myboardMain.do?option=${option}&pageNum=${startPage - 10}">[이전]</a>
                           </c:if> <c:forEach var="i" begin="${startPage}" end="${endPage}">
                              <a
                                 href="/semiProject/mj/myboardMain.do?option=${option}&pageNum=${i}">[${i}]</a>
                           </c:forEach> <c:if test="${endPage < pageCount}">
                              <a
                                 href="/semiProject/mj/myboardMain.do?option=${option}&pageNum=${startPage + 10}">[다음]</a>
                           </c:if></td>
                     </tr>
                  </c:if>
               </table>
